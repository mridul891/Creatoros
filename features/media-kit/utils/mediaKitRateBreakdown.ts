...
import { formatMoney } from "@/features/media-kit/utils/mediaKitFormatters";
...
export function getRateAddOns(rates: MediaKitFormData["rates"]) {
  const addOns: Array<{ label: string; multiplier: number }> = [];
  const inclusions: string[] = [];

  if (rates.addOns.paidAdUsage.enabled) {
    const months = rates.addOns.paidAdUsage.durationMonths;
  ...
  if (rates.addOns.noCompetitorWork.enabled) {
    const months = rates.addOns.noCompetitorWork.durationMonths;
  ...
  if (rates.addOns.adsFromHandle.enabled) {
    addOns.push({
...
export function buildRateInclusions(
  ...
  const { inclusions } = getRateAddOns(rates);
  const lines = inclusions.map((html) => ({ html }));

  if (deliverableTitles.some((title) => /ugc/i.test(title))) {
    lines.push({
      html:
        "<b>UGC</b> is shot and edited for you to post on your own channels. It is not published on my page, so it does not include access to my audience.",
    });
  }
  ...
  if (lines.length === 0) {
    lines.push({
      html:
        "<b>Organic posting only.</b> Paid ad usage, exclusivity and whitelisting are quoted separately.",
    });
  }
