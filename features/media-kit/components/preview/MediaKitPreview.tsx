...
import "@/features/media-kit/styles/media-kit-preview.css";
import {
  formatCategoryLabel,
  formatCompactNumber,
  formatGenderSplit,
...
} from "@/features/media-kit/utils/mediaKitFormatters";
import { getEngagementRate } from "@/features/media-kit/utils/mediaKitStats";
import {
  buildRateCards,
  buildRateInclusions,
  formatRateMoney,
} from "@/features/media-kit/utils/mediaKitRateBreakdown";
...
export function MediaKitPreview() {
  ...
  const genderSplit = formatGenderSplit(audience?.womenPercentage ?? 0);
  const audienceItems = [
  ...
  const workItems =
    work?.items?.filter((item) => item.title.trim()) ?? [];
  const brands = parseCommaSeparatedValues(work?.brandsWorkedWith);

  const rateCards = rates ? buildRateCards(rates) : [];
  const deliverableTitles = rateCards.map((card) => card.title);
  const inclusions = rates
    ? buildRateInclusions(rates, deliverableTitles)
    : [];
  ...
  const statItems = [
    ...
    {
      value: formatCompactNumber(stats?.avgReelViews ?? 0),
    ...
    {
      value: formatCompactNumber(stats?.avgStoryViews ?? 0),
      label: "Story views",
    },
  ];
    ...
    <div className="media-kit-preview">
      ...
      <article className="mk-kit">
              ...
              {rateCards.map((card) => (
                <div key={card.title} className="mk-rate-card">
                  ...
                  <div className="mk-rate-price">
                    {formatRateMoney(card.total, rates?.currency ?? "INR")}
                  </div>
                  ...
                  {card.addOnLines.length > 0 ? (
                    <div className="mk-rate-breakdown">
                      <div className="mk-rate-line">
                        <span>Base</span>
                        <span>
                          {formatRateMoney(card.base, rates?.currency ?? "INR")}
                        </span>
                      </div>
                    ...
                    <div className="mk-rate-breakdown">Organic posting only</div>
                  )}
                </div>
              ))}
            </div>
        ...
        <footer className="mk-updated">
          <span>Updated {formatUpdatedDate()}</span>
