...
import { TASK_PRIORITIES } from "@/features/tasks/enums/task"
...
export const campaignTemplateCreateUpdateSchema = z.object({
  ...
  description: z
    .string()
    .trim()
    .max(1000, "Template description cannot exceed 1000 characters.")
    .optional(),
  tasks: z
    .array(campaignTemplateTaskSchema)
    .min(1, "Add at least one task to this template.")
    .max(30, "Templates can include up to 30 tasks."),
  deliverables: z
    .array(campaignTemplateDeliverableSchema)
    .max(20, "Templates can include up to 20 deliverables."),
})

export type CampaignTemplateCreateUpdateInput = z.infer<
  typeof campaignTemplateCreateUpdateSchema
>
