...
import { TASK_STATUS_LABEL, TASK_STATUSES } from "@/features/tasks/enums/task"
        ...
        <Table className="min-w-[860px]">
          <TableHeader>
            <TableRow className="border-border border-b hover:bg-transparent">
              ...
              <TableHead className="hidden min-w-[90px] px-4 font-mono text-[10px] text-muted-foreground tracking-wider lg:table-cell">
                Order
              </TableHead>
          ...
          <TableBody>
            {items.map((item) => {
              return (
                <TableRow
                  key={item.id}
                  className="border-border border-b hover:bg-muted"
                >
                  ...
                  <TableCell className="px-4">
                    <select
                      ...
                      onChange={(event) =>
                        onStatusChange(
                          item.id,
                          event.target.value as TaskListItem["status"]
                        )
                      }
                  ...
                  <TableCell className="px-4">
                    <span
                      className={`inline-flex rounded-full border px-2 py-1 font-semibold text-[11px] ${getPriorityClass(item.priority)}`}
                    >
                      {item.priority}
                    </span>
                  </TableCell>
                  ...
                  <TableCell className="hidden px-4 text-[12px] text-muted-foreground lg:table-cell">
                    {item.orderIndex}
                  </TableCell>
