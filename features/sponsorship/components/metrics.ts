...
export function calculatePipelineSummary(deals: Deal[]): PipelineSummary {
  ...
  const totalSigned = deals
    ...
    .reduce((sum, deal) => sum + deal.value, 0)
  const plannedCount = deals.filter(
    (deal) => deal.stage === SponsorshipStage.LEAD
  ).length
  const shootingCount = deals.filter(
    (deal) => deal.stage === SponsorshipStage.OUTREACH
  ).length
  const pendingReviewCount = deals.filter(
    (deal) => deal.stage === SponsorshipStage.SIGNED
  ).length
  const publishedCount = deals.filter(
    (deal) => deal.stage === SponsorshipStage.PAID
  ).length
  ...
  const highPriorityOpenCount = deals.filter(
    (deal) =>
      deal.priority === DealPriority.HIGH &&
      deal.stage !== SponsorshipStage.PAID
  ).length
  ...
  const signedPct =
    deals.length > 0
      ? Math.round(
          (deals.filter(
            (deal) =>
              deal.stage === SponsorshipStage.SIGNED ||
              deal.stage === SponsorshipStage.PAID
          ).length /
            deals.length) *
            100
