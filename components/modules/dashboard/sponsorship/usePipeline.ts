...
import { DealPriority, SponsorshipMode, SponsorshipStage } from "@/enums/sponsorship";
import type { Deal } from "@/types/sponsorship";
import { SEED_DEALS } from "./data";
import { calculatePipelineSummary } from "./metrics";
import { STAGES, type ModalState, type Stage } from "./shared";
...
export function usePipeline(initialMode: SponsorshipMode = SponsorshipMode.TABLE) {
  ...
  const filteredDeals = useMemo(() => {
    ...
    return deals.filter(
      (deal) => deal.brand.toLowerCase().includes(query) || deal.category.toLowerCase().includes(query),
    );
  ...
  const handlePriorityChange = useCallback((id: number, priority: DealPriority) => {
    setDeals((prev) => prev.map((deal) => (deal.id === id ? { ...deal, priority } : deal)));
  }, []);
  ...
  const onDragLeaveStage = useCallback((stage: Stage) => {
    setUiState((prev) => (prev.dragOverStage === stage ? { ...prev, dragOverStage: null } : prev));
  }, []);
