...
import type { FunnelDatum } from "../sponsorship/metrics";
import { STAGE_BG22_CLASS, STAGE_CFG, STAGE_TEXT_CLASS, fmt } from "../sponsorship/shared";
    ...
    <div className="rounded-2xl border border-border bg-card px-[22px] py-5">
      ...
      <div className="flex flex-col gap-[6px]">
        {data.map((item, index) => {
          ...
          const convRate = index === 0 ? null : prevCount > 0 ? Math.round((item.count / prevCount) * 100) : 0;
            ...
            <div key={item.stage}>
              {convRate !== null && (
                <div className="mb-[3px] flex items-center gap-[6px] pl-2">
                  ...
                  <span
                    className={`font-mono text-[9px] font-bold ${convRate >= 70 ? "text-[#16a34a]" : convRate >= 40 ? "text-[#d97706]" : "text-[#E8402A]"}`}
                  >
                    {convRate}% conversion
                  </span>
                </div>
              ...
              <div className="flex items-center gap-[10px]">
                <div className="flex w-[90px] shrink-0 items-center gap-[6px]">
                  <stageCfg.icon size={11} color={stageCfg.color} />
                  <span className={`font-mono text-[10px] font-semibold ${STAGE_TEXT_CLASS[item.stage]}`}>
                    {item.stage}
                  </span>
                </div>
                <div className="h-[22px] flex-1 overflow-hidden rounded-[5px] bg-muted">
                  ...
                  >
                    <span className={`whitespace-nowrap font-mono text-[9px] font-bold ${STAGE_TEXT_CLASS[item.stage]}`}>
                      {item.count}
                    </span>
                  </div>
                </div>
                <div className="w-14 shrink-0 text-right font-mono text-[10px] font-bold text-muted-foreground">
                  {fmt(item.value)}
                </div>
              </div>
            </div>
