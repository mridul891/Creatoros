...
import { SEED_DEALS, MONTHLY_DATA } from "../sponsorship/data";
import {
...
const RevenueByCategory = dynamic(
  ...
  {
    ssr: false,
    loading: () => <div className="h-[240px] rounded-2xl border border-border bg-card" />,
  },
);
...
export function DashboardOverview() {
  ...
  const revenueTrend = useMemo(() => calculateRevenueTrendMetrics(MONTHLY_DATA), []);

  return (
    <div className="w-full max-w-[1300px] px-9 py-7">
      <div className="mb-6">
        <h1 className="mb-1 text-2xl font-extrabold tracking-[-0.04em] text-foreground">Dashboard Overview</h1>
        <div className="text-[13px] text-muted-foreground">
          Executive snapshot of pipeline value, stage health, and revenue momentum
        </div>
      </div>
