...
import type { AnalyticsFiltersState, AnalyticsUiState } from "@/types/analytics"
    ...
    <div className="">
      {uiState.viewMode === AnalyticsViewMode.SYNC_FAILED && (
        <div className="flex items-center gap-[10px] border-b border-b-[rgba(232,64,42,0.18)] bg-[rgba(232,64,42,0.08)] px-8 py-[10px]">
          <HugeiconsIcon icon={AlertCircleIcon} size={13} color={ACCENT} />
          <span className="font-medium text-[#E8402A] text-[13px]">
            Couldn&apos;t sync data — showing last good snapshot from 6h ago.
          </span>
          <button className="ml-2 cursor-pointer rounded-[7px] border border-[rgba(232,64,42,0.3)] bg-transparent px-[10px] py-[3px] text-[#E8402A] text-xs">
            Retry
          </button>
          <button
            onClick={() =>
