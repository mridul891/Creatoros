...
export const mediaKitAudienceSchema = z.object({
  topAgeGroups: z.string().trim().min(1, "Age group is required"),
...
export const mediaKitWorkSchema = z.object({
  ...
  brandsWorkedWith: z.string().max(1000, "Brand list is too long"),
})

export const mediaKitDeliverableSchema = z.object({
  title: z.string().trim().min(1, "Deliverable name is required").max(100),
...
export const mediaKitRatesSchema = z.object({
  ...
  paymentTerms: z.string().trim().min(1, "Payment terms are required").max(500),
})
